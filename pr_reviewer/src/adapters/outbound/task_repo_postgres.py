...
class PostgresTaskRepo(ITaskRepo):
    ...
    def add(self, repo_url: str, pr_number: int, auth_token: Optional[str]=None) -> int:
        task_id = 0
        with Session(self.db) as session:
            task = Task(repo_url=repo_url, pr_number=pr_number, status=TaskStatus.PENDING, auth_token=auth_token or "")
    ...
    def update(self, task_id:int, status: TaskStatus, result: Optional[Review]=None) -> None:
        with Session(self.db) as session:
            stmt = select(Task).where(Task.id == task_id)
