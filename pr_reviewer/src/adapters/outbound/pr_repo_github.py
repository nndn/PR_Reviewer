...
class GithubPrRepo(IPullRequestRepo):
    ...
    def get(self, repo_url: str, pr_number: int, auth_token: Optional[str]=None) -> PullRequest:
        ...
        while len(curr_page) > 0:
            for file in curr_page:
                ...
                if len(file.patch) > 0 and len(file.patch) < MAX_FILE_CHANGES_CHAR_SIZE:
                    ...
                    file_changes.append(FileChanges(patch=patch, file_name=file.filename, patch_start_line_number=line_number))
            curr_page_no += 1
            curr_page = pr.get_files().get_page(curr_page_no)
        
        return PullRequest(assignees=assignees, title=pr.title, description=pr.body, file_changes=file_changes)
